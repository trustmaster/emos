...
Five capabilities exist — `issue_tracker`, `code_host`, `wiki`, `chat`, `docs` —
and `.agents/config.yaml` → `tools:` declares which provider fills each one:

```yaml
tools:
  issue_tracker: jira        # jira | github | none
  code_host:     none         # github | none  (PR review + code browsing)
  wiki:          confluence   # confluence | none
  chat:          slack        # slack | none
  docs:          gws          # gws | none
```

One provider can fill more than one capability: `github` fills `issue_tracker`
(via `create-github-issue`) and `code_host` (PR review + browsing), and its two
sections activate independently — e.g. `issue_tracker: jira` + `code_host: github`.
...
`/jira-sync`, `/create-github-issue`, `/github-sync`, `/pr-review`,
`/github-browse`, `/confluence-load`, `/confluence-save`, `/slack-capture`, and
the `/gws-*` family — see `.agents/integrations/`. (`github-pr-fetch` is an
internal transport skill that `pr-review` calls; you don't invoke it directly.)
